	...
	<head>
		...
		$(document).ready(function(){
			...
			$("#deleteHaulBtn").click(function(){
				window.location.href="<%=basePath%>admin/userAdmin?method=deleteUser&userId="+$("#deleteUserId").val();
			});
			...
			$("#btnDelBatch").click(function(){
				...
				//执行删除操作
				window.location.href="<%=basePath%>admin/userAdmin?method=deleteUserBatch&userIds="+ids;
			});
		});
		...
		function gotoPage(currPage){
			window.location.href=
				"<%=basePath%>admin/userAdmin?method=gotoUserManage&currPage="+currPage
					...
					<div class="panel panel-default" style="margin-top:5px;">
						...
						<div class="panel-body">
							<div class="col-md-12 column">
								<form class="form-horizontal" action="admin/userAdmin?method=gotoUserManage" method="post">
									<div class="row clearfix">
										...
										<div class="col-md-3 column">
											<input type="button" value="新增用户" 
												onclick="javascript:location.href='<%=basePath%>admin/userAdmin?method=gotoAddUser'" 
								...
								<table class="table table-hover" style="margin-top:2px auto;">
									...
									<tbody>
										<c:forEach var="user" items="${ requestScope.userList }" varStatus="vs">
											<tr>
												<td>
													<c:if test="${ sessionScope.loginUser.username eq user.username }">
														<c:set var="display" value="none" /> 
														<c:set var="color" value="#ff0000" />
													</c:if>
													<c:if test="${ sessionScope.loginUser.username ne user.username }">
														<c:set var="display" value="inline" />
														<c:set var="color" value="#000000" />
													</c:if>
													<input type="checkbox" id="inlineCheckbox${ user.userId }" 
													name="chk"
													value="${ user.userId }" style="display:${ display };">
												</td>
